module.exports = {
  pathPrefix: '/', // Prefix for all links. If you deploy your site to example.com/portfolio your pathPrefix should be "portfolio"

  siteTitle: 'Ben Cahn Photography', // Navigation and Site Title
  siteTitleAlt: 'Ben Cahn Photography', // Alternative Site title for SEO
  siteTitleShort: 'Ben', // short_name for manifest
  siteHeadline: 'Publishing & Creating stunning photos', // Headline for schema.org JSONLD
  siteUrl: 'https://bencahn.com', // Domain of your site. No trailing slash!
  siteLanguage: 'en', // Language Tag on <html> element
  siteLogo: '/src/favicon_camera.png', // Used for SEO and manifest
  siteDescription: 'Dark One-Page portfolio with cards & detailed project views',
  author: 'LekoArts', // Author for schema.org JSONLD

  // siteFBAppID: '123456789', // Facebook App ID - Optional
  userTwitter: '@BarackObama', // Twitter Username
  ogSiteName: 'emilia', // Facebook Site Name
  ogLanguage: 'en_US', // og:language
  googleAnalyticsID: 'UA-47519312-4',

  // Manifest and Progress color
  themeColor: '#3498DB',
  backgroundColor: '#2b2e3c',

  // Your information
  avatar: '/logos/ben_cahn_logo_small.png',
  name: 'Ben Cahn',
  location: 'Berkeley, CA',
  socialMedia: [
    {
      url: 'https://twitter.com',
      name: 'Twitter',
    },
    {
      url: 'https://www.instagram.com/bendreamin/?hl=en',
      name: 'Instagram',
    },
    {
      url: 'https://www.bencahn.com',
      name: 'Homepage',
    },
  ],
}
